 ...
 *
 * A PROMPT_COMMAND hook re-runs the resolution on every prompt, so a
 * running shell self-heals — even after a mid-session update — with zero
 * visible output.
 */
...
# Resolve now, then on every prompt, so a running shell recovers even after
# a mid-session VS Code update — with zero visible output.
_persisterm_refresh
case "$PROMPT_COMMAND" in
  *_persisterm_refresh*) ;;  # already installed
  *) PROMPT_COMMAND="_persisterm_refresh\${PROMPT_COMMAND:+;$PROMPT_COMMAND}" ;;
esac
`;
